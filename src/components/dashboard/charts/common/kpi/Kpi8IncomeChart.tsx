...
// Bar = avg salary jt, Line = % lulusan dengan gaji ≥ 1.2× UMP
const defaultAvg = [
      ...
      <div className="h-80">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={avgData} margin={{ top: 30, right: 50, left: 20, bottom: 30 }}>
            ...
            <YAxis yAxisId="left" tickFormatter={(v) => `${v} jt`} fontSize={13} domain={[0, 12]} stroke={C.blue}
              label={{ value: "Rata-rata Gaji (Juta Rp)", angle: -90, position: "insideLeft", fontSize: 12, fill: C.blue }} />
            <YAxis yAxisId="right" orientation="right" tickFormatter={(v) => `${v}%`} fontSize={13} domain={[0, 100]} stroke={C.red}
            ...
            <Line yAxisId="right" type="monotone" dataKey="pctAbove" name="% ≥ 1,2× UMP" stroke={C.red} strokeWidth={2.5}
              dot={{ r: 5, fill: C.red }} />
            <ReferenceLine yAxisId="right" y={lam.threshold} stroke={C.red} strokeDasharray="6 3" strokeWidth={2}
      ...
      <div className="h-80">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={distData} margin={{ top: 30, right: 20, left: 20, bottom: 30 }}>
            ...
            <XAxis dataKey="year" fontSize={13} stroke="hsl(var(--muted-foreground))"
              label={{ value: "Tahun Kelulusan", position: "insideBottom", offset: -8, fontSize: 12, fill: "hsl(var(--muted-foreground))" }} />
            <YAxis tickFormatter={(v) => `${v}%`} fontSize={13} domain={[0, 100]}
